...
| ✅ FATTO | ~~**FloatingActionButton**~~ | Implementato (04/07/2026) come **slot dello Scaffold** (approccio B, non widget-foglia): posizione riferita alla pagina via `FloatingActionButtonLocation`. Sottosistema completo sul modello di BottomNavigationBar: `FabData` in RootScreenJsonData, `appStore.fabClass` (@observable, MobX rigenerato), `FabData`/`JSON_FAB_DATA`, `WidgetTypeFABLayout`, accept-logic in `on_accept_widgets`, render+location in preview e in `view_screen.dart`, code-gen `floatingActionButton:`+`floatingActionButtonLocation:`, nodo tree-view, right-click, property view con dropdown Posizione. `fab_class.dart`+`fab_property_view.dart`; test in `test/template_test.dart` |
