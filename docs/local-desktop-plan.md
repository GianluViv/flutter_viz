...
### Fase 1 — Compilabilità desktop (rimuovere i plugin web-only) 🟡 IN PARTE COMPLETATA
...
- [ ] **YouTube / Video / Audio player — NON ancora rimossi.** Erano ancora presenti in
      `baseWidgetsList` (`lib/widgets/widgets.dart`) al termine della Fase 0 e la build Windows
      completava comunque: a differenza di `google_maps`, questi plugin (`youtube_player_iframe`,
      `just_audio`, `video_player`) non hanno bloccato la compilazione nativa nei test fatti finora
      (potrebbero comunque non funzionare a runtime su desktop). Restano da rimuovere per
      completare questa fase secondo la decisione presa (vedi §6 per le alternative future).
...
- [ ] Aggiungere `path_provider` a `pubspec.yaml` (cartella dati applicazione) — non ancora fatto.
- [ ] Rimuovere youtube/video/audio dalla palette e dal dispatcher (vedi sopra).
- [ ] Rimuovere le dipendenze relative da `pubspec.yaml`: `youtube_player_iframe`, `just_audio`,
      `video_player`, `image_picker_web`, `g_recaptcha_v3` (+ `rxdart` se usato solo dall'audio).
      (`google_maps` e `web` già rimossi.)
...
- **Verifica:** `flutter build windows` completa senza errori (**confermato**); `flutter build linux`
  non ancora testato (nessun host Linux disponibile). Palette ancora da ripulire da youtube/video/audio.

### Fase 2 — `LocalProjectService` (persistenza su file)
**Scopo:** salvare/caricare un progetto su disco.
- [ ] Definire il modello `Project` e (de)serializzazione `.fvproj` (§2.1).
- [ ] Implementare `LocalProjectService` (§2.2), registrarlo in `get_it`.
- [ ] Usare `file_picker` (già dipendenza) per dialoghi Apri/Salva-con-nome desktop e
      `path_provider` per la cartella di default `<AppData>/FlutterViz/`.
- [ ] Test unitari sul round-trip: `Project` → JSON → `Project` e integrazione con
      `widgetClassToJsonData()`/`applyScreenJsonToView()` su una schermata di esempio.
- **Verifica:** test verdi; un progetto scritto e riletto ricostruisce l'albero identico.
...
- [ ] Rimuovere `LoginScreen`, `RegisterScreen`, `ForgotPassword`, Firebase Auth (+
      `firebase_core`/`firebase_auth`/`firebase_analytics`), `google_sign_in`, recaptcha, e il
...
- [ ] **Media**: sostituire upload/lista media remoti con copia dei file immagine nella cartella
      del progetto (`media/`) e riferimenti relativi; adattare `media_component.dart`.
...
## 5. Rischi principali
- Compilazione desktop bloccata dai plugin web-only → mitigato rimuovendoli in Fase 1 (punto critico).
